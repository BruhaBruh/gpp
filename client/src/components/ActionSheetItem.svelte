<script lang="ts">
	...
	import type { IconColor } from './SidebarNavigationItem.svelte';
...
{#if to}
	<a
		{...$$restProps}
		class={[
			'hover:text-gray-50 hover:bg-gray-700 flex items-center p-2 transition-colors ease-in text-gray-400 focus:ring-2 focus:ring-primary-600',
			$$restProps.class
		]
			.filter(Boolean)
			.join(' ')}
